...
class Inference:
    ...
    def __init__(
            self, path: Optional[str] = None, config: Optional[InferConfig] = None
    ):
        ...
        self.model.state = read_ckpt_lowmem(self.model.state, path, self.params.cores_per_replica, load_opt=False)
    ...
    def generate_tokens(
        ...
        source = jnp.array(np.pad(prompt, ((0, 0), (self.params.seq - prompt.shape[1], 0),)))
        sampler_options = {
